...
class NotificaAuction extends Notification
{
    use Queueable;
    ...
    {
        //  Storage::get('public/pdf/TRAMITE_SUBASTA-' . $id . '.pdf');
        // ->attach('/path/to/file', [
        //     'as' => 'name.pdf',
        //     'mime' => 'application/pdf',
        // ]);
        return (new MailMessage)
            ->subject('ADJUDICACIÓN DE SUBASTA NÚMERO: '. 
            $this->msg[0]->id.'  '.
            $this->msg[1]->nombremarca.' | '.
            $this->msg[1]->nombremodeloauto.' | '.
            $this->msg[1]->anno)
            ->line('Estimado cliente: '. $this->msg[2]->firstName .' '. $this->msg[2]->lastName .'.')
            ->line('Nos es grato informarle que has sido el ganador en de la subasta: '.
            ...
            ->line('Por un monto de: $ '. $this->msg[0]->puja_actual .' ('.$this->msg[3].' 00/100 M.N.)')
            ->line($this->msg[7] ? 'Descuento: '. $this->msg[6]->descuento_percent .' %' : 'Esta subasta no posee descuento' )
            ->line($this->msg[7] ? 'Por un monto final de: $ '. $this->msg[5].' ('.$this->msg[4].' 00/100 M.N.)' :'Por un monto final de: $ '. $this->msg[0]->puja_actual .' ('.$this->msg[3].' 00/100 M.N.)' )
            ->line('Subasta realizada el día: '. $this->msg[0]->final)
            ->line('-------------------------------------------------------------------------')
            ...
            ->attach(storage_path('app/public/pdf/TRAMITE_SUBASTA-' . $this->msg[1]->id . '.pdf'), [
                     'as' => 'TRAMITE_SUBASTA-' . $this->msg[1]->id . '.pdf',
                     'mime' => 'application/pdf',
                 ])
            ->attach(storage_path('app/public/pdf/TRAMITE_SUBASTA-' . $this->msg[1]->id . '.jpg'), [
                'as' => 'TRAMITE_SUBASTA-' . $this->msg[1]->id . '.jpg',
                'mime' => 'image/jpg',
            ]);
